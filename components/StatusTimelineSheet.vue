...
// Load timeline based on props
watch(() => [props.itemId, props.itemType, props.status], () => {
  if (props.itemId) {
    isLoading.value = true
    
    if (props.itemType.toLowerCase() === 'cusdec') {
      title.value = `CusDec ${props.itemId} Status Timeline`
      
      if (props.status?.toLowerCase() === 'rejected') {
        statusItems.value = getRejectedTimeline()
      } else {
        // Reset to default timeline if not rejected
        statusItems.value = [
          {
            title: 'Declaration Submitted',
            date: getRelativeDate(3),
    ...
    } else if (props.itemType.toLowerCase() === 'container') {
      title.value = `Container ${props.itemId} Status Timeline`
      statusItems.value = getContainerTimeline()
    } else {
      title.value = `${props.itemType.toUpperCase()} ${props.itemId} Status`
    }
  ...
  <Sheet :open="isSheetOpen" @update:open="isSheetOpen = $event">
    <SheetContent side="right" class="timeline-sheet w-full sm:max-w-md md:max-w-lg flex flex-col h-full overflow-hidden">
      <SheetHeader class="timeline-header mb-6 flex-shrink-0">
        ...
        <div v-if="props.status" class="mt-4">
          <div 
            class="status-badge inline-flex items-center px-3 py-1 text-sm font-medium rounded-full"
            :class="{
              'bg-green-100 text-green-700 border border-green-200': getStatusColor(props.status) === 'success',
              'bg-blue-100 text-blue-700 border border-blue-200': getStatusColor(props.status) === 'info',
              'bg-amber-100 text-amber-700 border border-amber-200': getStatusColor(props.status) === 'warning',
              'bg-purple-100 text-purple-700 border border-purple-200': getStatusColor(props.status) === 'purple',
              'bg-red-100 text-red-700 border border-red-200': getStatusColor(props.status) === 'error',
              'bg-gray-100 text-gray-700 border border-gray-200': getStatusColor(props.status) === 'default'
            }"
          >
            <span class="status-dot" 
              :class="{
                'bg-green-500': getStatusColor(props.status) === 'success',
                'bg-blue-500': getStatusColor(props.status) === 'info',
                'bg-amber-500': getStatusColor(props.status) === 'warning',
                'bg-purple-500': getStatusColor(props.status) === 'purple',
                'bg-red-500': getStatusColor(props.status) === 'error',
                'bg-gray-500': getStatusColor(props.status) === 'default'
              }"
            ></span>
            {{ props.status }}
          </div>
