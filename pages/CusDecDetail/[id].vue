...
const features = computed(() => {
  ...
  return {
    submission: true,
    assessment: status !== 'waiting confirmation',
    payment: ['processing', 'exited'].includes(status),
    release: status === 'exited'
  }
})

// Fetch CusDec details from backend
onMounted(async () => {
  console.log('CusDecDetail mounted, route params:', route.params);
  ...
  setTimeout(() => {
    ...
    // If the ID is 2 or 3, set status to Waiting Confirmation for testing the accept/disclaim buttons
    const status = ['2', '3'].includes(id) ? 'Waiting Confirmation' : 'Processing'
...
// Handle accept CusDec
const handleAccept = () => {
  // In a real app, this would call an API
  alert('CusDec accepted successfully!')
  // Update status and redirect
  cusdec.value.status = 'Processing'
  setTimeout(() => {
    router.push('/CusDecTracking')
  }, 1000)
}

// Handle disclaim CusDec
const handleDisclaim = () => {
  // In a real app, this would call an API
  if (confirm('Are you sure you want to disclaim this CusDec?')) {
    alert('CusDec disclaimed successfully!')
  ...
  <div class="min-h-screen pb-20 bg-gray-50">
    ...
    <div class="container px-4 mx-auto">
      ...
      <div v-else class="mt-4">
        <!-- Action Buttons for Waiting Confirmation status -->
        <div v-if="cusdec.status.toLowerCase() === 'waiting confirmation'" class="flex gap-3 mb-4">
          <ButtonComponent 
            color="success" 
            class="flex-1"
            @click="handleAccept"
